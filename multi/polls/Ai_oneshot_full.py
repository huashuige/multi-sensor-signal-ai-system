...
AiSampleMode = ctypes.c_char(1)   #设置采集模式  0为连续采集模式  1为oneshot模式
AiOneShotPoints = ctypes.c_uint(100)   #oneshot模式下 获取的点数
...
print(temp)
temp = dll.SetUSB5AiOneShotPoints(DevIndex, AiOneShotPoints) #设置oneshot模式下 获取的点数
print(temp)
...
################################################################################################
#oneshot模式下采集到设定的点数后 即采集结束
# Getai函数的返回值 代表采集卡缓冲区还有多少数据点  正常情况下返回值是一个大于等0切不随时间增大而增大的数值 如果随时间增大而增大 则代表获取的数据点少或执行效率低
temp9= dll.USB5GetAi(DevIndex,AiOneShotPoints, Ai.ctypes.data_as(POINTER(c_float)), TimeOut)
...
dll.USB5CloseDevice(DevIndex)       #关闭采集卡
